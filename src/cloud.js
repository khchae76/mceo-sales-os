...
function hasMeaningfulCloudData(data) {
  return Boolean(data) && ['mceo-atass-v1', 'mceo-clv-builder-v1', 'mceo-closer-disc-v1', 'mceo-objection-disc-v1']
    .some((key) => data[key])
}
...
async function handleSession(session) {
  ...
  try {
    ...
    } else if (!hasMeaningfulLocalData() && hasMeaningfulCloudData(workspace.data) && !sessionStorage.getItem('mceo-cloud-auto-loaded')) {
      sessionStorage.setItem('mceo-cloud-auto-loaded', '1')
      await loadSnapshot({ silent: true })
    } else if (!hasMeaningfulLocalData()) {
      setStatus('登录成功；云端暂无可载入的记录，可以直接开始使用。')
    } else {
